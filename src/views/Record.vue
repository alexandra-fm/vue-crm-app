...
export default {
  ...
  metaInfo() {
    return { title: localizeFilter("RecordTitle") };
  },
  async mounted() {
    ...
    async submitHandler() {
      ...
      if (this.canCreateRecord) {
        try {
          ...
          this.$message(localizeFilter("Message_RecordAdded") + `${bill}`);

          this.$v.$reset(), (this.amount = 0), (this.description = "");
          // eslint-disable-next-line no-empty
        } catch (e) {}
      } else {
        this.$message(
          localizeFilter("Message_RecordAddedError") + 
